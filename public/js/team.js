...
(async function() {
    await zyq.init();
    zyq.menu([db.BEHEERDER, "backup uitslagen van alle ronden" , async function () {
...
async function uitslagenTeam(kop, rondenTabel) {
    ...
    for (const team of teams) {
        if (team.teamCode === zyq.o_o_o.team) {
            kop.innerHTML = [zyq.teamVoluit(zyq.o_o_o.team), zyq.seizoenVoluit(zyq.o_o_o.seizoen), team.omschrijving].join(zyq.SCHEIDING);
            break;
        }
    }
...
function uitslagenTeamPerRonde(u, rondeNummer, rondenTabel) {
    if (u) { // eventueel ronde overslaan, wegens oneven aantal teams in een poule
        ...
        rondenTabel.appendChild(zyq.htmlRij(u.ronde.rondeNummer, datumKolom, zyq.naarTeam(u.ronde), uitslagKolom));
        if (u.uitslagen.length) {
            const div = document.getElementById("ronde" + rondeNummer); // 9 x div met id="ronde1".."ronde9"
            div.appendChild(document.createElement("h2")).innerHTML = ["Ronde " + rondeNummer, datumKolom].join(zyq.SCHEIDING);
            const tabel = div.appendChild(document.createElement("table"));
            tabel.appendChild(zyq.htmlRij("", zyq.wedstrijdVoluit(u.ronde), "", uitslagKolom));
